...
const db = new DatabaseSync(path.join(DATA_DIR, 'hr.db'));
...
if (userCount === 0) {
  db.prepare('INSERT INTO users (username, password, name, role, company_key, created_at) VALUES (?,?,?,?,?,?)')
...
function getUserFromToken(req) {
  ...
  return db.prepare('SELECT id, username, name, role FROM users WHERE id = ?').get(row.user_id) || null;
}
function logActivity(user, action, candidate, detail) {
  try {
    db.prepare('INSERT INTO activities (user_id, user_name, action, candidate_id, candidate_name, detail, created_at) VALUES (?,?,?,?,?,?,?)')
...
const server = http.createServer(async (req, res) => {
  try {
    ...
    if (pathname === '/api/auth/register' && req.method === 'POST') {
      ...
      const info = db.prepare('INSERT INTO users (username, password, name, role, company_key, created_at) VALUES (?,?,?,?,?,?)')
      ...
      db.prepare('INSERT INTO sessions (token, user_id, expires_at) VALUES (?,?,?)').run(token, info.lastInsertRowid, exp);
      return sendJSON(res, 200, { token, user: { id: info.lastInsertRowid, username: b.username, name: b.name, role: 'member' } });
    }
    ...
    if (pathname === '/api/candidates' && req.method === 'GET') {
      ...
      const rows = db.prepare(`SELECT * FROM candidates ${where} ORDER BY updated_at DESC`).all(...params);
    ...
    if (pathname === '/api/candidates' && req.method === 'POST') {
      ...
      const info = db.prepare(`INSERT INTO candidates
      ...
      const cand = db.prepare('SELECT * FROM candidates WHERE id = ?').get(info.lastInsertRowid);
      logActivity(user, '新增候选人', cand, `添加候选人 ${cand.name}（${STAGES[stage]}）`);
    ...
    if (m) {
      ...
      if (req.method === 'PUT') {
        ...
        db.prepare(`UPDATE candidates SET name=?,gender=?,phone=?,email=?,position=?,source=?,stage=?,owner_id=?,owner_name=?,expected_salary=?,current_org=?,education=?,interview_note=?,notes=?,tags=?,expected_onboard_date=?,updated_at=? WHERE id=?`)
        ...
        const nc = db.prepare('SELECT * FROM candidates WHERE id = ?').get(id);
        logActivity(user, '更新候选人', nc, `更新 ${nc.name} 的资料`);
      ...
      if (req.method === 'DELETE') {
        ...
        logActivity(user, '删除候选人', c, `删除候选人 ${c.name}`);
        return sendJSON(res, 200, { ok: true });
      }
    }
    ...
    if (mAdv && req.method === 'POST') {
      ...
      db.prepare('UPDATE candidates SET stage=?, status=?, hired_at=?, interview_note=COALESCE(?,interview_note), updated_at=? WHERE id=?')
        .run(stage, status, hiredAt, b.interview_note || null, now(), id);
      const nc = db.prepare('SELECT * FROM candidates WHERE id = ?').get(id);
      logActivity(user, '推进阶段', nc, `${c.name} 进入「${STAGES[stage]}」`);
      if (status === 'hired') ensureOnboarding(nc);
      return sendJSON(res, 200, { candidate: nc });
    }
    ...
    if (mBack && req.method === 'POST') {
      ...
      db.prepare('UPDATE candidates SET stage=?, status=?, updated_at=? WHERE id=?').run(stage, 'active', now(), id);
      const nc = db.prepare('SELECT * FROM candidates WHERE id = ?').get(id);
      logActivity(user, '退回阶段', nc, `${c.name} 退回至「${STAGES[stage]}」`);
      return sendJSON(res, 200, { candidate: nc });
    }
    ...
    if (mPool && req.method === 'POST') {
      ...
      if (!c) return sendJSON(res, 404, { error: '未找到' });
      const b = await readBody(req);
      db.prepare('UPDATE candidates SET status=?, tags=COALESCE(?,tags), notes=COALESCE(?,notes), updated_at=? WHERE id=?')
        .run('in_pool', b.tags || null, b.notes || null, now(), id);
      const nc = db.prepare('SELECT * FROM candidates WHERE id = ?').get(id);
      logActivity(user, '转入人才库', nc, `${c.name} 进入人才库`);
      return sendJSON(res, 200, { candidate: nc });
    }
    ...
    if (mRej && req.method === 'POST') {
      ...
      const c = db.prepare('SELECT * FROM candidates WHERE id = ?').get(id);
      if (!c) return sendJSON(res, 404, { error: '未找到' });
      db.prepare('UPDATE candidates SET status=?, updated_at=? WHERE id=?').run('rejected', now(), id);
      const nc = db.prepare('SELECT * FROM candidates WHERE id = ?').get(id);
      logActivity(user, '淘汰候选人', nc, `${c.name} 被标记为淘汰`);
      return sendJSON(res, 200, { candidate: nc });
    }
    ...
    if (mHire && req.method === 'POST') {
      ...
      const c = db.prepare('SELECT * FROM candidates WHERE id = ?').get(id);
      if (!c) return sendJSON(res, 404, { error: '未找到' });
      db.prepare('UPDATE candidates SET status=?, stage=?, hired_at=?, updated_at=? WHERE id=?')
        .run('hired', STAGES.length - 1, now(), now(), id);
      const nc = db.prepare('SELECT * FROM candidates WHERE id = ?').get(id);
      logActivity(user, '标记为入职', nc, `${c.name} 标记为已入职`);
      ensureOnboarding(nc);
      return sendJSON(res, 200, { candidate: nc });
    }
    ...
    if (mRe && req.method === 'POST') {
      ...
      const c = db.prepare('SELECT * FROM candidates WHERE id = ?').get(id);
      if (!c) return sendJSON(res, 404, { error: '未找到' });
      db.prepare('UPDATE candidates SET status=?, updated_at=? WHERE id=?').run('active', now(), id);
      const nc = db.prepare('SELECT * FROM candidates WHERE id = ?').get(id);
      logActivity(user, '重新激活', nc, `${c.name} 重新进入招聘流程`);
      return sendJSON(res, 200, { candidate: nc });
    }

    // Onboarding list
    if (pathname === '/api/onboarding' && req.method === 'GET') {
      const rows = db.prepare('SELECT * FROM onboarding ORDER BY created_at DESC').all();
    ...
    if (mOnb) {
      ...
      if (req.method === 'GET') {
        const o = db.prepare('SELECT * FROM onboarding WHERE id = ?').get(id);
      ...
      if (req.method === 'PUT') {
        ...
        db.prepare('UPDATE onboarding SET items=?, handler_id=?, handler_name=?, completed_at=? WHERE id=?')
        ...
        const no = db.prepare('SELECT * FROM onboarding WHERE id = ?').get(id);
        logActivity(user, '更新入职办理', { name: no.candidate_name }, `更新 ${no.candidate_name} 的入职清单`);
    ...
    if (pathname === '/api/stats' && req.method === 'GET') {
      const all = db.prepare('SELECT * FROM candidates').all();
...
function ensureOnboarding(c) {
  const exists = db.prepare('SELECT id FROM onboarding WHERE candidate_id = ?').get(c.id);
  if (exists) return;
  const defaultItems = [
    '签订劳动合同', '收集身份证/学历证复印件', '办理工牌', '开通邮箱/企业微信账号',
    '入职培训', '分配工位与设备', '社保公积金开户'
  ].map((label, i) => ({ key: 'item' + i, label, done: false }));
  db.prepare('INSERT INTO onboarding (candidate_id, candidate_name, position, handler_id, handler_name, items, started_at, created_at) VALUES (?,?,?,?,?,?,?,?)')
...
server.listen(PORT, HOST, () => {
  console.log(`HR Talent System running at http://${HOST}:${PORT}`);
});
